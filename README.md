...
| `test\compare-reference.ps1` | do the numbers agree with a separate implementation, written in Python, that shares none of this code? (708 comparisons across nineteen test frames, 0 failures; the thirty-two that stay unanswered are the ones the Python declines to cover, plus paths neither side exercises -- and each says so in its own line rather than being excused here) |
