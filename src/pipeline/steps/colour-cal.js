...
// The selected star median has to stand this far above the sky before a ratio
// taken against it means anything. Section 2.2: the ratio is
// (median - pedestal) / (reference - pedestal), and as the numerator shrinks
// toward the pedestal the quotient stops being a measurement and starts being
// a division of two small differences of two large numbers.
var CC_PEDESTAL_HEADROOM = 3.0;
...
function stepColourCal(img, params, report){
  ...
  if (count < effective.minStarPixels){
    ...
    var lowest = Math.min(starMedians[0], starMedians[1], starMedians[2]);
    var floor = CC_PEDESTAL_HEADROOM * Math.max(pedestals[0], pedestals[1], pedestals[2]);
    var refMedian = above[refIdx];

    if (!(lowest >= floor)){
      stars.skipReason = 'the faintest star median is ' + lowest.toFixed(6) +
        ', less than ' + CC_PEDESTAL_HEADROOM + 'x the sky pedestal (' +
        Math.max(pedestals[0], pedestals[1], pedestals[2]).toFixed(6) +
        '); a ratio taken above the sky is unstable when the star is barely above it';
    } else if (!(refMedian > 0)){
