 ...
 * Quatro niveis, e a diferenca entre eles muda o que a proxima pessoa faz:
 *
 *   MEDIDO       lido de um arquivo que o proprio programa gravou, com o texto
 *                verbatim -- um arquivo de parceiro, ou uma saida da medicao de
 *                escritores (pixels sinteticos, header escrito pelo programa)
 *   OBSERVADO EM EXEMPLO PUBLICO
 *                alguem publicou o header; ninguem aqui abriu o arquivo nem
 *                conferiu que ele nao foi editado. Mais fraco que MEDIDO
 *   DOCUMENTADO  esta escrito na documentacao do programa
 *   SUPOSTO      ninguem verificou; e o que se acredita
 *
 * A MEDICAO DE ESCRITORES (medicao-escritores.md, parte A) mediu as operacoes
 * tonais do Siril por comando, em duas versoes: 1.2.1 (Ubuntu 24.04, o ensaio) e
 * 1.4.4 (Windows, siril-cli). A entrada e a mesma byte a byte nas duas, entao
 * toda diferenca e do Siril. Relatorio: medicao-escritores-relatorio-a.txt.
 * Os textos citados abaixo sao o que cada versao gravou; o que diverge entre as
 * duas esta dito na entrada.
 *
 * Estado hoje: SEIS MEDIDAS -- uma delas tambem REFUTADA para o modo de
 * visualizacao --, duas SUPOSTO. Escopo do MEDIDO: Siril, estas duas versoes,
 * operacao por COMANDO. Os dialogos (parte B) e a cadeia com o GraXpert (parte C)
 * nao foram medidos, e nada aqui vale para eles.
 *
 * UM ROTULO POR LINHA. Cada linha de HISTORY recebe no maximo UM rotulo: o da
 * primeira regra que casa NELA. Medido: o Siril grava uma operacao por linha, e
 * a linha `GHS asinh (...)` do asinh modificado casava ao mesmo tempo em `asinh`
 * e em `GHS` -- dupla contagem de uma operacao so. Por isso A ORDEM IMPORTA: a
 * regra mais especifica vem antes da mais geral (`GHS asinh` antes de `asinh` e
 * de `GHS`).
 *
 * E os rotulos saem na ordem DESTA TABELA, nao na ordem das linhas do arquivo:
 * o log que os lista nao muda de ordem conforme o escritor ordenou o HISTORY.
 */
var STRETCH_HISTORY = [
  /* MEDIDO PARA O COMANDO, REFUTADO PARA O MODO DE VISUALIZACAO -- os dois,
   * e a distincao e a entrada inteira.
   *
   * O COMANDO `autostretch` se declara, com o mesmo texto nas duas versoes:
   *
   *     Siril 1.2.1 e 1.4.4   Autostretch (shadows: -2.80, target bg: 0.25, unlinked)
   *                           Autostretch (shadows: -2.80, target bg: 0.25, linked)
   *
   * O MODO DE VISUALIZACAO do Siril 1.4.4 chamado "autostretch" NAO toca nos
   * pixels e nao deixa rastro -- medido num arquivo de parceiro: olhar em
   * autostretch e salvar devolve a mediana original, 0,001177. E o esticamento
   * DE VERDADE feito pelo dialogo de histograma grava outra coisa (entrada 2):
   *
   ...
   *
   * O botao de visualizacao, o comando e o dialogo tem nomes parecidos e deixam
   * rastros diferentes. Botao de auto-stretch DENTRO do dialogo de histograma:
   * nao medido (parte B).
   */
   ...
   *
   * Foi a primeira entrada MEDIDA da tabela, e por muito tempo a unica: casa os
   * dois arquivos de parceiro, e a medicao de escritores nao a exercitou (o
   * dialogo de histograma e parte B).
   *
  ...
  /* MEDIDO -- Siril 1.2.1 e 1.4.4, o mesmo texto nas duas. ENTRADA NOVA.
   *
   *     GHS asinh (pivot: 0.004, amount: 53.60 [0.00, 1.00])
   *
   * E o asinh modificado do Siril (comando `modasinh`). Vem ANTES das regras de
   * `asinh` e de `GHS` porque a linha casa nas duas: sem ela, uma operacao so
   * recebia dois rotulos. Fixture: `fixture-escritor-ghs-asinh` recebe
   * exatamente um rotulo, este.
   */
  [/GHS asinh/i,              'Modified asinh'],
  /* MEDIDO -- Siril 1.2.1 e 1.4.4, o mesmo texto nas duas:
   *
   *     Asinh stretch (amount: 150.0, offset: 0.0, human: no)
   *     Asinh stretch (amount: 150.0, offset: 0.0, human: yes)
   *
   * O OFFSET DECLARADO NAO E O APLICADO: o comando recebeu 0.001, a linha diz
   * `0.0`, e o offset foi aplicado -- com e sem ele as medianas saem 0,1085 e
   * 0,1299, com o mesmo HISTORY. Mesma familia do arredondamento de tres casas:
   * a declaracao diz QUE houve esticamento, nunca reconstroi QUAL.
   *
   * A palavra `asinh` tambem pode aparecer em contexto que nao e esticamento;
   * isso nao foi medido.
   */
  [/asinh/i,                  'Asinh stretch'],
  /* MEDIDO -- Siril 1.2.1 e 1.4.4. Tres textos, e a regra mudou por causa deles.
   *
   *     GHS (pivot: 0.004, amount: 53.60, local: 6.0 [0.00, 1.00])     casa
   *     AutoGHS (linked, k.sigma: 0.00, amount: 5.00, local: 13.0 [0.00, 0.70])
   *                                                                  casa AGORA
   *     GHS BP shift (new BP: 0.001)                                   NAO casa
   *
   * A regra antiga, `\bGHS\b`, errava nos dois sentidos:
   *
   *   `AutoGHS`       nao tem fronteira de palavra antes de `GHS`, entao uma
   *                   operacao que ESTICA passava sem rotulo. Fixture:
   *                   `fixture-escritor-autoghs` recebe `GHS`.
   *   `GHS BP shift`  e o deslocamento do ponto preto -- operacao AFIM, que
   *                   preserva a classe (1 - r2 = 1,6e-15 contra a base). Era
   *                   FALSO POSITIVO num controle. Hoje o custo seria trocar o
   *                   limiar de 0,05 por 0,02; depois do item 2 o HISTORY decide
   *                   sozinho, e o falso positivo viraria veredito. Fixture:
   *                   `fixture-escritor-controle-bp` nao recebe rotulo nenhum.
   *
   * O `AutoGHS` MUDOU DE SENTIDO entre as versoes: os mesmos argumentos esticam
   * bem menos no 1.4.4 (mediana do canal 0: 0,298 no 1.2.1, 0,075 no 1.4.4), e a
   * linha declara `amount: 5.00` contra `147.41` (= e^5 - 1). MEDIDO: o efeito.
   * SUPOSTO: que o segundo argumento passou de D para amount. No 1.2.1 a linha
   * era longa demais e o Siril a cortou em dois cartoes, com o `)` no segundo;
   * o termo esta no comeco, entao o corte nao o alcanca.
   *
   * O nome por extenso (`generalised hyperbolic`) continua SUPOSTO: nenhuma
   * versao medida o escreve.
   */
  [/generalised hyperbolic|generalized hyperbolic|\b(?:auto)?GHS\b(?!\s+BP shift)/i, 'GHS'],
  /* MEDIDO -- Siril 1.2.1 e 1.4.4, o mesmo texto nas duas:
   *
   *     Midtones transfer (0.003, 0.0123, 1.000)
   *
   * Comando `mtf`. Os tres numeros sao preto, meio-tom e branco, e o arredondamento
   * da entrada 2 vale aqui tambem.
   */
  [/midtone/i,                'Midtones transfer'],
  // SUPOSTO, E A MAIS ARRISCADA DAS OITO -- e o risco esta medido, nao suposto.
  //
  // `curves` e a unica das oito que NAO e termo tecnico: e palavra comum em
  ...
  [/\bcurves?\b/i,            'Curves'],
  // SUPOSTO -- o asinh modificado do Siril com o NOME DO COMANDO no texto.
  //
  // Nenhuma versao medida escreve `modasinh`: o 1.2.1 e o 1.4.4 gravam
  // `GHS asinh (...)`, que a entrada nova, acima, reconhece. Esta fica para um
  // escritor que use o nome do comando, e sem evidencia de que ele exista.
  //
  // O ROTULO ESTAVA ERRADO: era `Autostretch`, para uma regra que casava
  // `modasinh`. E a alternativa `|autostretch` saiu -- era codigo morto por
  // construcao, porque a regra 1 casa a mesma palavra com o mesmo rotulo.
  [/modasinh/i,               'Modified asinh']
];

// A regra que fica com uma linha de HISTORY: a primeira da tabela que casa
// nela, ou -1. Ver UM ROTULO POR LINHA, no comentario de STRETCH_HISTORY.
function historyRule(line){
  for (var hi = 0; hi < STRETCH_HISTORY.length; hi++){
    if (STRETCH_HISTORY[hi][0].test(line)) return hi;
  }
  return -1;
}

// Os rotulos que um HISTORY declara: um por linha, na ordem DA TABELA, sem
// repetir -- duas regras podem dar o mesmo rotulo.
//
// Funcao, e nao laco dentro do pipeline, porque o teste de texto do catalogo
// (test/compare-catalogo.ps1) chama ESTA funcao com cada linha medida. Um teste
// que rodasse uma copia da tabela provaria a copia.
function historyLabels(lines){
  var seen = {};
  for (var li = 0; li < lines.length; li++){
    var r = historyRule(lines[li]);
    if (r >= 0) seen[STRETCH_HISTORY[r][1]] = true;
  }
  var out = [];
  for (var hj = 0; hj < STRETCH_HISTORY.length; hj++){
    var lab = STRETCH_HISTORY[hj][1];
    if (seen[lab] && out.indexOf(lab) < 0) out.push(lab);
  }
  return out;
}
...
async function openFile(buffer, fileName, opts, post){
  ...
  var historyHits = historyLabels(hdu.history);
