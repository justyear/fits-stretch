...
if ($Only -eq 'all' -or $Only -eq 'crop') {
    ...
    function CropCards([string]$obj, [double[]]$geom, [int]$n, [string[]]$extra) {
        $c = @(
            ...
            (Hists 'CROP WIDTH IS ODD (1601) on purpose: (1601-1) is a')
            (Hists 'CROP multiple of the background lattice divisor, and that')
            (Hists 'CROP is the case that produced a NaN in the last column.')
